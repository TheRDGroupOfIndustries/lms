  ...
  return (
    <section className="py-12 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="space-y-6 md:space-y-8 m-4 md:m-6"> {/* Adjusted margin for smaller devices */}
          {features.map((feature, index) => (
            <div key={index} className="flex items-center bg-gray-50 rounded-lg p-4 md:p-6 transition-transform transform hover:scale-105"> {/* Reduced padding for mobile */}
              <Image
                src={feature.image}
                alt={feature.title}
                width={400}
              ...
              />
              <div className="w-2/3 md:w-1/2"> {/* Adjusted width for mobile */}
                <h3 className="text-[18px] md:text-[24px] font-semibold mb-1">{feature.title}</h3> {/* Adjusted title size */}
                <p className="text-gray-600 mb-3 text-[12px] md:text-[18px]">{feature.description}</p> {/* Adjusted description size */}
                <button className="text-green-500 font-semibold flex items-center hover:underline text-[10px] md:text-[14px]"> {/* Adjusted button size */}
